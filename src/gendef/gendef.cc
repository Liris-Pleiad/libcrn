  ...
  while( !feof(dump)) {
	  ...
	  if( fgets( buf, 64999, dump ) != NULL ) {
		  if(!strstr(buf," UNDEF ") && strstr(buf," External ")
					...
					&& !strstr(buf, "vfprintf_l")
					&& !strstr(buf, "vscprintf")
					&& !strstr(buf, "vscprintf_l")
					&& !strstr(buf, "vsnprintf_l")
					&& !strstr(buf, "vsnprintf_s_l")
					&& !strstr(buf, "scanf_options")
					&& !strstr(buf, "vsscanf_s_l")
					&& !strstr(buf, "isa_available")
					///
					) {
			  char *s = strchr(buf,'|') + 1;
			  while(*s == ' ' || *s == '\t') s++;
