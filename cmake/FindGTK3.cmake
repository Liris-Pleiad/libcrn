...
# other optional components like gtkmm.
#
...
#        find_package(GTK3 2.6 REQUIRED gtk)
#
#
#
# ::
...
#        find_package(GTK3 2.10 REQUIRED gtk)
#
#
#
# ::
...
#        find_package(GTK3 2.8 COMPONENTS gtk gtkmm)
#
#
#
# ::
...
# Version 1.6-libcrn2 (Yann LEYDIER <yann@leydier.info>)
#   * Converted to GTK3 (removed glade)
# Version 1.6-libcrn (Yann LEYDIER <yann@leydier.info>)
...
#   * 14260: Export a GTK3_DEFINITIONS variable to set /vd2 when appropriate
#            (i.e. MSVC)
...
#   * Remove GTK3_SKIP_MARK_AS_ADVANCED option, as now the variables are
...
#   * 12596: Missing paths for FindGTK3 on NetBSD
#   * 12049: Fixed detection of GTK include files in the lib folder on
#            multiarch systems.
# Version 1.3 (11/9/2010) (CMake 2.8.4)
#   * 11429: Add support for detecting GTK3 built with Visual Studio 10.
...
#   * Add support for detecting GTK3 under macports (thanks to Gary Kramlich)
...
#     via the GTK3_ADDITIONAL_SUFFIXES variable (kind of a future-kludge in
...
#   * Prefaced internal variables with _GTK3 to prevent collision
#   * Changed internal macros to functions
#   * Enhanced documentation
# Version 0.6 (1/8/08)
#   Added GTK3_SKIP_MARK_AS_ADVANCED option
...
function(_GTK3_FIND_LIBRARY _var _lib _expand_vc _append_version)
    ...
    if(_expand_vc AND MSVC)
        ...
        elseif(MSVC11)
            ...
            set(_library_d ${_library}-d)
        elseif(MSVC14)
            #set(_library ${_library}-vc140)
            set(_library_d ${_library}-d.dll)
            set(_library ${_library}.dll)
        endif()
    ...
    if(_append_version)
        ...
        endforeach()
				list(APPEND _lib_list  "${_library}")
				list(APPEND _libd_list "${_library_d}")
    else()
...
function(_GTK3_ADD_TARGET _var)
    ...
    if(GTK3_${_var}_FOUND AND NOT TARGET GTK3::${_basename})
        ...
        foreach(_dep ${_${_var}_GTK3_DEPENDS})
            ...
            message("Not adding ${_basename} because ${_dep} not found")
                return()
            endif()
        endforeach()
...
foreach(_GTK3_component ${GTK3_FIND_COMPONENTS})
    if(_GTK3_component STREQUAL "gtk")
        ...
        _GTK3_FIND_LIBRARY    (PANGOXFT pangoxft true true)
        ...
        if(UNIX)
            ...
            _GTK3_FIND_LIBRARY    (GTK gtk true true)
        endif()
        if(UNIX)
            ...
            _GTK3_ADD_TARGET (GTK GTK3_DEPENDS gdk atk pango gdk_pixbuf gthread gobject glib
                              GTK3_OPTIONAL_DEPENDS gio pangocairo cairo)
        endif()
    ...
    elseif(_GTK3_component STREQUAL "gtkmm")
        ...
        if(UNIX)
            ...
            _GTK3_ADD_TARGET      (GDKMM GTK3_DEPENDS pangomm gtk glibmm sigc++ gdk atk gdk_pixbuf pango gobject glib
                                     GTK3_OPTIONAL_DEPENDS giomm cairomm gio pangocairo cairo
                                     OPTIONAL_INCLUDES ${FREETYPE_INCLUDE_DIR_ft2build} ${FREETYPE_INCLUDE_DIR_freetype2}
                                                       ${GTK3_FONTCONFIG_INCLUDE_DIR}
                                                       ${GTK3_X11_INCLUDE_DIR})
        ...
        if(UNIX)
            ...
            _GTK3_ADD_TARGET      (GTKMM GTK3_DEPENDS atkmm gdkmm pangomm gtk glibmm sigc++ gdk atk gdk_pixbuf pango gthread gobject glib
                                     GTK3_OPTIONAL_DEPENDS giomm cairomm gio pangocairo cairo
                                     OPTIONAL_INCLUDES ${FREETYPE_INCLUDE_DIR_ft2build} ${FREETYPE_INCLUDE_DIR_freetype2}
                                                       ${GTK3_FONTCONFIG_INCLUDE_DIR}
                                                       ${GTK3_X11_INCLUDE_DIR})
